...
func TestRenderGitLabPipeline(t *testing.T) {
	...
	out, err := Render("templates/ci/gitlab-ci-orobox.yml.tmpl", NewCIData("1.0.0-rc31", deployConfig()))
	if err != nil {
	...
	for _, want := range []string{
		...
		"releases/download/1.0.0-rc31/orobox_Linux_x86_64",
...
func TestRenderGitLabPipelineWithoutDeployStages(t *testing.T) {
	...
	out, err := Render("templates/ci/gitlab-ci-orobox.yml.tmpl", NewCIData("1.0.0-rc31", nil))
	if err != nil {
		t.Fatalf("Render() error = %v", err)
	}
	rendered := string(out)
...
func TestRenderGitLabRootIncludesTheOroboxPipeline(t *testing.T) {
	...
	out, err := Render("templates/ci/gitlab-ci.yml.tmpl", NewCIData("1.0.0-rc31", nil))
	if err != nil {
		t.Fatalf("Render() error = %v", err)
	}
