...
Orobox is a command-line tool (CLI) developed in Go to quickly set up an isolated and reproducible development environment for OroCommerce. It supports the development of individual **bundles**.
...
```bash
curl -sSfL "https://github.com/algoritma-dev/orobox/releases/download/0.0.18-dev/orobox_$(uname -s)_$(uname -m | sed 's/aarch64/arm64/')" -o ~/.local/bin/orobox && chmod +x ~/.local/bin/orobox && hash -r
```
...
```powershell
mkdir -Force "$HOME\.local\bin"; iwr "https://github.com/algoritma-dev/orobox/releases/download/0.0.18-dev/orobox_Windows_$($env:PROCESSOR_ARCHITECTURE.ToLower().Replace('amd64','x86_64')).exe" -OutFile "$HOME\.local\bin\orobox.exe"
```
...
- `type`: Defines the installation type (currently only `bundle` is supported in the main branch).
- `class`: Name of the bundle class.
- `namespace`: PHP namespace of the bundle.
